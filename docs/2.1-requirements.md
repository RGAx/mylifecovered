...
- Should the Total Coverage input on the Life Insurance Child-Level landing for **I Know How Much I Need** be a text box or a dropdown? If dropdown, needed is a list of value options and a default value option.
- When dragging sliders on Quote - Recommended, does total coverage amount need to change? I'm sure it does, and that it's part of an API call.
  - If so, Mindtree will need to add this functionality
- What are the minimun, maximum, beginning, and increment values on the Quote - Recommended sliders? Put another way, what's the lowest and highest each slider can go, where should it start, and how much should it jump by when the slider is moved?
- What is the correct phone number for the four Quote - Recommended carriers on the right?